...
// for Me card support
void SeasidePeopleModel::createMeCard(QContact me)
{
  ...
  if (!me.saveDetail(&name))
    ...
    if (!me.saveDetail(&fav))
        qWarning() << "[SeasidePeopleModel] failed to save mecard favorite to " << fav.isFavorite();
...
int SeasidePeopleModel::columnCount(const QModelIndex& parent) const
{
    Q_UNUSED(parent);
    return 1;
}
...
QVariant SeasidePeopleModel::data(int row, int role) const
{
    ...
    default:
        qWarning() << "[SeasidePeopleModel] request for data with unknown row" <<
                      row << " role : " << role;
        return QVariant();
    }
}

void SeasidePeopleModel::fixIndexMap()
{
    int i=0;
    beginResetModel();
...
void SeasidePeopleModel::addContacts(const QList<QContact> contactsList,
                              int size)
{
...
void SeasidePeopleModel::contactsAdded(const QList<QContactLocalId>& contactIds)
{
...
void SeasidePeopleModel::onAddedFetchChanged(QContactAbstractRequest::State requestState)
{
...
void SeasidePeopleModel::contactsChanged(const QList<QContactLocalId>& contactIds)
{
    if (contactIds.size() == 0)
        return;
...
void SeasidePeopleModel::onChangedFetchChanged(QContactAbstractRequest::State requestState)
{
    QContactFetchRequest *fetchRequest = checkRequest<QContactFetchRequest>(sender(), requestState);
    if (!fetchRequest)
        return;
...
void SeasidePeopleModel::contactsRemoved(const QList<QContactLocalId>& contactIds)
{
...
void SeasidePeopleModel::dataReset()
{
...
void SeasidePeopleModel::onDataResetFetchChanged(QContactAbstractRequest::State requestState)
{
    QContactFetchRequest *fetchRequest = checkRequest<QContactFetchRequest>(sender(), requestState);
    if (!fetchRequest)
        return;
...
bool SeasidePeopleModel::createPersonModel(QString avatarUrl, QString thumbUrl, QString firstName, QString firstPro,
...
void SeasidePeopleModel::deletePerson(const QString& uuid)
{
...
void SeasidePeopleModel::editPersonModel(QString uuid, QString avatarUrl, QString firstName, QString firstPro,
                                  QString lastName, QString lastPro, QString companyname,
                                  QStringList phonenumbers, QStringList phonecontexts, bool favorite,
                                  QStringList accounturis, QStringList serviceproviders, QStringList emailaddys,
                                  QStringList emailcontexts, QStringList street, QStringList city, QStringList state,
                                  ...
                                  QStringList urllinks,  QStringList urlcontexts, QDate birthday, QString notetext)
{
    ...
    if (contact.isEmpty()) {
       qWarning() << "[SeasidePeopleModel] Unable to find contact, cannot complete edit on uuid " << uuid;
       return;
    }
    ...
    if ((name.firstName() != firstName) || (name.lastName() != lastName)) {
        ...
        if (!contact.saveDetail(&name))
            qWarning() << "[SeasidePeopleModel] failed to update name";
    }
    ...
    if (priv->localeHelper->getCountry() == QLocale::Japan) {
        ...
        if (needsUpdate == true) {
            ...
            if (!contact.saveDetail(&nickname))
                qDebug() << "[SeasidePeopleModel] failed to update nickname";
        }
    }
    ...
    if (company.name() != companyname) {
        ...
        if (!contact.saveDetail(&company))
            qDebug() << "[SeasidePeopleModel] failed to update company";
    }
    ...
    foreach (QContactDetail detail, contact.details<QContactPhoneNumber>()) {
        if (!contact.removeDetail(&detail))
            qWarning() << "[SeasidePeopleModel] failed to remove phone number";
    }
    ...
    foreach (QContactDetail detail, contact.details<QContactOnlineAccount>()) {
        if (!contact.removeDetail(&detail))
            qWarning() << "[SeasidePeopleModel] failed to remove im account";
    }
    ...
    foreach (QContactDetail detail, contact.details<QContactEmailAddress>()) {
        if (!contact.removeDetail(&detail))
            qWarning() << "[SeasidePeopleModel] failed to remove email address";
    }
    ...
    foreach (QContactDetail detail, contact.details<QContactAddress>()) {
        if (!contact.removeDetail(&detail))
            qWarning() << "[SeasidePeopleModel] failed to remove addresses";
    }
...
void SeasidePeopleModel::setCurrentUuid(const QString& uuid)
{
    priv->currentGuid.setGuid(uuid);
    qDebug() << "sets current uuid to " << uuid << "test " << priv->currentGuid.guid();
}

void SeasidePeopleModel::toggleFavorite(const QString& uuid)
{
    QContactLocalId id = priv->uuidToId[uuid];
    QContact &contact = priv->idToContact[id];
 ...
 */
void SeasidePeopleModel::queueContactSave(QContact contactToSave)
{
...
void SeasidePeopleModel::savePendingContacts()
{
...
void SeasidePeopleModel::onSaveStateChanged(QContactAbstractRequest::State requestState)
{
 ...
 */
void SeasidePeopleModel::removeContact(QContactLocalId contactId)
{
...
void SeasidePeopleModel::onRemoveStateChanged(QContactAbstractRequest::State requestState)
{
...
// For Me card support
void SeasidePeopleModel::onMeFetchRequestStateChanged(QContactAbstractRequest::State requestState)
{
...
bool SeasidePeopleModel::isSelfContact(const QContactLocalId id) const
{
    if (!priv->manager->hasFeature(QContactManager::SelfContact, QContactType::TypeContact))
        return false;
...
bool SeasidePeopleModel::isSelfContact(const QUuid id) const
{
    return isSelfContact(priv->uuidToId[id]);
}

bool SeasidePeopleModel::isSelfContact(const QString id) const
{
    QUuid uuid(id);
    return isSelfContact(priv->uuidToId[uuid]);
}

void SeasidePeopleModel::fetchOnlineOnly(const QVariantList &stringids)
{
